...
output_csv = "features_windowed_yawn_distinct.csv"

fps = 6
window_size_sec = 120  # 2 minutes
stride_sec = 30        # 1 minute
...
for idx, (video, frames) in enumerate(video_data.items(), 1):
    ...
    for i in range(0, len(frames) - window_size + 1, stride):
        ...
        # Count distinct blinks based on EAR threshold with refractory gap
        blink_count = 0
        blinking = False
        last_blink_end = -999
        MIN_BLINK_GAP = 6  # ~1 second (6 FPS)
